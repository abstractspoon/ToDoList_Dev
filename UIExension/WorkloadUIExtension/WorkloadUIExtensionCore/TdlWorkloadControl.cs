...
namespace WorkloadUIExtension
{
	...
	public class WorkloadTaskItem
	{
		...
		public WorkloadTaskItem(String label)
		{
			m_Title = label;
			m_TaskID = 0;
            m_ParentID = 0;
		...
		public WorkloadTaskItem(Task task)
		{
		...
		public void FixupParentID(WorkloadTaskItem parent)
		{
	...
	public class TdlWorkloadControl : WorkloadControl
	{
		...
		private Dictionary<UInt32, WorkloadTaskItem> m_Items;
		...
		public TdlWorkloadControl(Translator trans, UIExtension.TaskIcon icons)
		{
			m_Trans = trans;
			m_TaskIcons = icons;

			m_SelectionRect = new UIExtension.SelectionRect();
			m_Items = new Dictionary<UInt32, WorkloadTaskItem>();
		...
		protected WorkloadTaskItem TaskItem(TreeNode node)
		{
			if (node == null)
				return null;

			return (ItemData(node) as WorkloadTaskItem);
		}

		protected WorkloadTaskItem TaskItem(Object itemData)
		{
			if (itemData == null)
				return null;

			return (itemData as WorkloadTaskItem);
		}
		...
		protected void RebuildTreeView(TaskList tasks)
		{
			...
			if (taskIsRoot)
			{
                var taskItem = new WorkloadTaskItem(task);
			...
			else
			{
				...
				rootNode = AddRootNode(new WorkloadTaskItem(m_Trans.Translate("Root")));
		...
		{
			if (dropPos == WorkloadControl.DropPos.On)
		...
		protected override Boolean DoDrop(WorkloadDragEventArgs e)
		{
                                              ...
                                              Font nodeFont, Object itemData)
		{
			...
			if (!taskColor.IsEmpty)
			{
				...
				{
					if (nodeState != WorkloadControl.NodeDrawState.None)
		...
		protected Boolean TaskHasIcon(WorkloadTaskItem taskItem)
		{
