...
namespace WorkloadUIExtension
{
	...
	public delegate Boolean DragDropChangeEventHandler(object sender, WorkloadDragEventArgs e);

	[System.ComponentModel.DesignerCategory("")]

	public class WorkloadDragEventItem
	{
		public WorkloadDragEventItem(TreeNode n)
		{
			if (n == null)
				return;

			var item = (n.Tag as WorkloadItem);
	...
	public class WorkloadDragEventArgs : EventArgs
	{
		public WorkloadDragEventArgs(TreeNode node, TreeNode parent, TreeNode sibling, bool copy)
		{
			dragged = new WorkloadDragEventItem(node);
			targetParent = new WorkloadDragEventItem(parent);
			afterSibling = new WorkloadDragEventItem(sibling);
			copyItem = copy;
		}

		public WorkloadDragEventItem dragged = null;
		public WorkloadDragEventItem targetParent = null;
		public WorkloadDragEventItem afterSibling = null;
	...
	public partial class WorkloadControl : UserControl
    {
		...
		private void DoDrop(TreeNode draggedNode, TreeNode dropTarget, DropPos dropPos, bool copy)
		{
			if (IsAcceptableDropTarget(draggedNode, dropTarget, dropPos, copy))
			{
                ...
                var args = new WorkloadDragEventArgs(draggedNode, parentNode, afterSiblingNode, copy);
		...
		virtual protected Boolean DoDrop(WorkloadDragEventArgs e)
		{
		...
		protected void EnsureItemVisible(WorkloadItem item)
        {
            if (item == null)
                return;
		...
		protected bool HandleCursorKey(Keys key)
		{
			...
			WorkloadItem rootItem = RootItem;
			WorkloadItem selItem = SelectedItem;
		...
		protected WorkloadItem SelectedItem
		{
			get
			{
				if (SelectedNode == null)
		...
		protected WorkloadItem Item(TreeNode node)
		{
			if (node == null)
				return null;

			return (node.Tag as WorkloadItem);
		}
		...
		protected WorkloadItem RootItem
		{
			get
			{
				if (IsEmpty())
        ...
        protected TreeNode AddNode(Object userData, TreeNodeCollection nodes, UInt32 uniqueID)
        {
            ...
            newNode.Tag = new WorkloadItem(userData);
        ...
        private Rectangle CalculateChildBoundingBox(TreeNode node)
        {
            ...
            if (node.IsExpanded)
            {
                foreach (TreeNode child in node.Nodes)
                {
                    WorkloadItem childItem = Item(child);
                    childBounds = WorkloadItem.Union(childBounds, childItem.TotalBounds);
                }
            }
		...
		private Rectangle CalculateTotalBoundingBox(TreeNode fromNode, TreeNode toNode)
		{
			...
			{
				WorkloadItem item = Item(node);
				totalBounds = WorkloadItem.Union(totalBounds, item.TotalBounds);
				...
				case DropPos.Above:
					{
						...
						{
							WorkloadItem prevItem = Item(node.PrevNode);
				...
				case DropPos.Below:
					{
						...
						{
							WorkloadItem nextItem = Item(node.NextNode);
		...
		private void DrawConnection(Graphics graphics, TreeNode nodeFrom, TreeNode nodeTo)
		{
			...
			WorkloadItem itemFrom = Item(nodeFrom);
			WorkloadItem itemTo = Item(nodeTo);
