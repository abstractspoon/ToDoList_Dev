...
public:
	...
	TASKCALITEM(const ITASKLISTBASE* pTasks, HTASKITEM hTask, const CSet<I_ATTRIBUTE>& attrib, DWORD dwCalcDates);

	TASKCALITEM& TASKCALITEM::operator=(const TASKCALITEM& tci);
	BOOL TASKCALITEM::operator==(const TASKCALITEM& tci);

	BOOL UpdateTask(const ITASKLISTBASE* pTasks, HTASKITEM hTask, const CSet<I_ATTRIBUTE>& attrib, DWORD dwCalcDates);
...
protected:
	void UpdateTaskDates(const ITASKLISTBASE* pTasks, HTASKITEM hTask, const CSet<I_ATTRIBUTE>& attrib, DWORD dwCalcDates);
	void ReformatName();
...
public:
	...
	BOOL Recalculate(const CTaskCalItemMap& mapData, I_ATTRIBUTE nAttrib, DWORD dwOptions);
