...
void CToDoCtrl::UpdateTask(TDC_ATTRIBUTE nAttrib, DWORD dwFlags)
{
	...
	if (nAttrib == TDCA_COMMENTS)
	{
		...
		SetModified(TRUE, TDCA_COMMENTS, aModTaskIDs);
...
BOOL CToDoCtrl::SetSelectedTaskCustomAttributeData(const CString& sAttribID, const TDCCADATA& data, BOOL bCtrlEdited)
{
	...
	if (nRes == SET_CHANGE)
	{
		TDC_ATTRIBUTE nAttrib = CTDCCustomAttributeHelper::GetAttributeID(sAttribID, m_aCustomAttribDefs);
 		SetModified(TRUE, nAttrib, aModTaskIDs);
...
BOOL CToDoCtrl::SetSelectedTaskMetaData(const CString& sKey, const CString& sMetaData)
{
	...
	if (nRes == SET_CHANGE)
 		SetModified(TRUE, TDCA_METADATA, aModTaskIDs);
...
BOOL CToDoCtrl::SetSelectedTaskColor(COLORREF color)
{
	...
	if (nRes == SET_CHANGE)
	{
		...
		SetModified(TRUE, TDCA_COLOR, aModTaskIDs);
	}
...
BOOL CToDoCtrl::SetSelectedTaskIcon(const CString& sIcon)
{
	...
	if (nRes == SET_CHANGE)
		SetModified(TRUE, TDCA_ICON, aModTaskIDs);
...
BOOL CToDoCtrl::SetSelectedTaskTitle(const CString& sTitle)
{
	...
	if (nRes == SET_CHANGE)
	{
		...
		SetModified(TRUE, TDCA_TASKNAME, aModTaskIDs);
	}
...
BOOL CToDoCtrl::SetSelectedTaskPriority(int nPriority)
{
	...
	if (nRes == SET_CHANGE)
	{
		...
		SetModified(TRUE, TDCA_PRIORITY, aModTaskIDs);
	}
...
BOOL CToDoCtrl::SetSelectedTaskRisk(int nRisk)
{
	...
	if (nRes == SET_CHANGE)
	{
		...
		SetModified(TRUE, TDCA_RISK, aModTaskIDs);
	}
...
BOOL CToDoCtrl::SetSelectedTaskFlag(BOOL bFlagged)
{
	...
	if (nRes == SET_CHANGE)
		SetModified(TRUE, TDCA_FLAG, aModTaskIDs);
...
BOOL CToDoCtrl::SetSelectedTaskLock(BOOL bLocked)
{
	...
	if (nRes == SET_CHANGE)
		SetModified(TRUE, TDCA_LOCK, aModTaskIDs);
...
BOOL CToDoCtrl::SetSelectedTaskDate(TDC_DATE nDate, const COleDateTime& date, BOOL bDateEdited)
{
	...
	if (nRes == SET_CHANGE)
	{
		...
		{
		case TDCD_CREATE:	
			SetModified(TRUE, TDCA_CREATIONDATE, aModTaskIDs); 
			break;
		...
		case TDCD_STARTTIME:
			...
			SetModified(TRUE, TDCA_STARTDATE, aModTaskIDs); 
			break;
		...
		case TDCD_DUETIME:
			...
			SetModified(TRUE, TDCA_DUEDATE, aModTaskIDs); 
			break;
				
		case TDCD_DONETIME:	
			SetModified(TRUE, TDCA_DONEDATE, aModTaskIDs); 
			break;
...
BOOL CToDoCtrl::SetSelectedTaskDone(const COleDateTime& date, BOOL bDateEdited)
{
	...
	if (nRes == SET_CHANGE)
	{
		...
		if (bSomeDone && !m_sCompletionStatus.IsEmpty())
		{
			...
			SetModified(TRUE, TDCA_STATUS, aModTaskIDs);
		}
	}
...
LRESULT CToDoCtrl::OnRecreateRecurringTask(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	if (aNewTaskIDs.GetSize())
	{
		...
		SetModified(TRUE, TDCA_NEWTASK, aNewTaskIDs);
...
BOOL CToDoCtrl::SetSelectedTaskPercentDone(int nPercent)
{
	...
	if (nRes == SET_CHANGE)
	{
		...
		SetModified(TRUE, TDCA_PERCENT, aModTaskIDs);
	}
...
BOOL CToDoCtrl::SetSelectedTaskCost(double dCost)
{
	...
	if (nRes == SET_CHANGE)
	{
		...
		SetModified(TRUE, TDCA_COST, aModTaskIDs);
	}
...
BOOL CToDoCtrl::SetSelectedTaskRecurrence(const TDCRECURRENCE& tr)
{
	...
	if (nRes == SET_CHANGE)
	{
		...
		SetModified(TRUE, TDCA_RECURRENCE, aModTaskIDs);
	}
...
BOOL CToDoCtrl::SetSelectedTaskTimeEstimate(double dTime, TDC_UNITS nUnits)
{
	...
	if (nRes == SET_CHANGE)
	{
		...
		SetModified(TRUE, TDCA_TIMEEST, aModTaskIDs);
...
BOOL CToDoCtrl::SetSelectedTaskTimeSpent(double dTime, TDC_UNITS nUnits)
{
	...
	if (nRes == SET_CHANGE)
	{
		...
		SetModified(TRUE, TDCA_TIMESPENT, aModTaskIDs);
	}
...
BOOL CToDoCtrl::SetSelectedTaskAllocBy(const CString& sAllocBy)
{
	...
	while (pos)
	{
		...
		switch (m_data.SetTaskAllocBy(dwTaskID, sAllocBy))
		{
			case SET_FAILED:
				return FALSE;
	...
	return SetTextChange(nRes, m_sAllocBy, sAllocBy, TDCA_ALLOCBY, IDC_ALLOCBY, aModTaskIDs, &m_cbAllocBy);
}
...
BOOL CToDoCtrl::SetSelectedTaskVersion(const CString& sVersion)
{
	...
	while (pos)
	{
		...
		switch (m_data.SetTaskVersion(dwTaskID, sVersion))
		{
			case SET_FAILED:
				return FALSE;
	...
	return SetTextChange(nRes, m_sVersion, sVersion, TDCA_VERSION, IDC_VERSION, aModTaskIDs, &m_cbVersion);
}
...
BOOL CToDoCtrl::SetSelectedTaskStatus(const CString& sStatus)
{
	...
	while (pos)
	{
		...
		switch (m_data.SetTaskStatus(dwTaskID, sStatus))
		{
			case SET_FAILED:
				return FALSE;
	...
	if (!SetTextChange(nRes, m_sStatus, sStatus, TDCA_STATUS, IDC_STATUS, aModTaskIDs, &m_cbStatus))
		return FALSE;
...
{
	CDWordArray aModTaskIDs;
	TDC_SET nRes = SetSelectedTaskArray(nAttrib, aItems, bAppend, aModTaskIDs);
										...
										BOOL bAppend, CDWordArray& aModTaskIDs)
{
	...
	while (pos)
	{
		...
		switch (m_data.SetTaskArray(dwTaskID, nAttrib, aItems, bAppend))
		{
			case SET_FAILED:
				return SET_FAILED;
...
BOOL CToDoCtrl::SetSelectedTaskArray(TDC_ATTRIBUTE nAttrib, const CCheckComboBox& combo)
{
	...
	while (pos)
	{
		...
		switch (m_data.SetTaskArray(dwTaskID, nAttrib, aTaskItems, FALSE))
		{
			case SET_FAILED:
				return FALSE;
...
BOOL CToDoCtrl::SetSelectedTaskFileRefs(const CStringArray& aFilePaths, BOOL bAppend, BOOL bCtrlEdited)
{
	...
	CDWordArray aModTaskIDs;
	TDC_SET nRes = SetSelectedTaskArray(TDCA_FILEREF, aFileRefs, bAppend, aModTaskIDs);
...
{
	CDWordArray aModTaskIDs;
	TDC_SET nRes = SetSelectedTaskArray(TDCA_DEPENDENCY, aDepends, bAppend, aModTaskIDs);
...
BOOL CToDoCtrl::SetSelectedTaskExternalID(const CString& sID)
{
	...
	return SetTextChange(nRes, m_sExternalID, sID, TDCA_EXTERNALID, IDC_EXTERNALID, aModTaskIDs);
}
								...
								BOOL bEdit, DWORD dwDependency = 0)
{
	...
	if (htiNew)
	{
		...
		SelectItem(htiNew);
		SetModified(TRUE, TDCA_NEWTASK, aModTaskIDs); 
...
BOOL CToDoCtrl::DeleteSelectedTask(BOOL bWarnUser, BOOL bResetSel)
{
	...
	SetModified(TRUE, TDCA_DELETE, aDelTaskIDs);
...
LRESULT CToDoCtrl::OnEditCancel(WPARAM /*wParam*/, LPARAM lParam)
{
	...
	else if (GetSelectedTaskID() == m_dwLastAddedID) 
	{
		...
		SetModified(TRUE, TDCA_DELETE, aModTaskIDs);
		UpdateControls();
	}
...
BOOL CToDoCtrl::SetCustomAttributeDefs(const CTDCCustomAttribDefinitionArray& aAttrib)
{
	...
	if (!Misc::MatchAllT(m_aCustomAttribDefs, aAttrib, FALSE))
	{
		...
		// update interface
		SetModified(TRUE, TDCA_CUSTOMATTRIBDEFS);
		UpdateControls(FALSE); // don't update comments
...
{
	SetModified(bMod, nAttrib, CDWordArray());
}
				...
				case ID_TDD_ADDTASKDEPENDENCY:
					{
						...
						SetModified(TRUE, TDCA_DEPENDENCY, aModTaskIDs);
					}
					break;
				...
				case ID_TDD_SETFILELINK:
					{
						...
						SetModified(TRUE, TDCA_FILEREF, aModTaskIDs);
					}
					break;
...
BOOL CToDoCtrl::MergeTasks(const CTaskFile& tasks, BOOL bMergeByID)
{
	...
	SetModified(TRUE, TDCA_PASTE, aTaskIDs);
	return TRUE;
}
...
BOOL CToDoCtrl::CopySelectedTaskAttributeData(TDC_ATTRIBUTE nFromAttrib, TDC_ATTRIBUTE nToAttrib)
{
	...
	if (nRes == SET_CHANGE)
	{
		SetModified(TRUE, nToAttrib, aModTaskIDs);
		UpdateControls(FALSE);
	}
