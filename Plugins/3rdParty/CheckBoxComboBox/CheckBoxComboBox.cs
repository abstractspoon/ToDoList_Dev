...
namespace PresentationControls
{
    ...
    public partial class CheckBoxComboBox : PopupComboBox
    {
            ...
            : base()
        {
            ...
            //ContainerControl.Padding = new Padding(4, 0, 0, 0);
        ...
        protected override void OnDropDownStyleChanged(EventArgs e)
        {
            ...
            _MustAddHiddenItem = HasHiddenItem;
        }
    ...
    public class CheckBoxComboBoxListControl : ScrollableControl
    {
		...
		protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
		{
			...
			{
			case Keys.Enter:
				_CheckBoxComboBox.HideDropDown();
				return true;

			case Keys.Escape:
				{
					// Restore original check states
					for (int i = 0; i < _Items.Count; i++)
					{
						_Items[i].Checked = _OriginalCheckStates[i];
					}
				}
		...
		public override bool PreProcessMessage(ref Message msg)
		{
			if (this.Visible)
			{
				...
				case NativeMethods.WM_KEYDOWN:
					{
						int selIndex = SelectedIndex, orgIndex = selIndex;
						int maxIndex = _Items.Count - 1;
						int minIndex = _CheckBoxComboBox.HasHiddenItem ? 1 : 0;
						...
						case Keys.PageDown:
							if (VScroll)
								selIndex += VerticalScroll.LargeChange;
							else
								selIndex = maxIndex;
							break;
						...
						case Keys.Space:
							...
							{
								// toggle check state
								_Items[selIndex].Checked = !_Items[selIndex].Checked;
							}
							break;
						}

						SelectedIndex = Math.Min(Math.Max(selIndex, minIndex), maxIndex);
					}
					break;
				}
			}
		...
		// hence it need pay no attention to the hidden first item
		private int SelectedIndex
		{
			...
			set
			{
				...
				if (value != selIndex)
				{
					...
					if (selIndex != -1)
					{
						_Items[selIndex].Selected = false;
						_Items[selIndex].Invalidate();
					}

					if (value >= 0 && value <= maxIndex)
					{
						_Items[value].Selected = true;
						_Items[value].Invalidate();
					}
				}
			}
		}

		private List<bool> _OriginalCheckStates = null;
		...
		protected override void OnVisibleChanged(EventArgs e)
        {
			...
			if (this.Visible)
			{
				// Snapshot the check states
				_OriginalCheckStates = new List<bool>();

				foreach (var item in _Items)
					_OriginalCheckStates.Add(item.Checked);

				// select the first item
				SelectedIndex = (_CheckBoxComboBox.HasHiddenItem ? 1 : 0);
			}
		...
		public void SynchroniseControlsWithComboBoxItems()
        {
			...
			bool hasHiddenItem = _CheckBoxComboBox.HasHiddenItem;
    ...
    public class CheckBoxComboBoxItemList : List<CheckBoxComboBoxItem>
    {
		...
		private void item_SelectionSet(object sender, EventArgs e)
		{
			int index = this.IndexOf(sender as CheckBoxComboBoxItem);

			// clear the selection of every item except this one
			for (int i = 0; i < Count; i++)
			{
				if (i != index && this[i].Selected)
				{
        ...
        public new void Add(CheckBoxComboBoxItem item)
        {
			...
			item.SelectionSet += new EventHandler(item_SelectionSet);
