...
namespace CustomComboBox
{
    ...
    public class CustomComboBox : ComboBox, IPopupControlHost
    {
        ...
        {
            DropDownControl.Size = CalcRequiredDropDownSize();
        }
		
		public virtual Size CalcRequiredDropDownSize()
		{
			if (DropDownControl != null)
			{
				...
				case SizeMode.UseComboSize:
					return new Size(Width, m_sizeCombo.Height);

				case SizeMode.UseControlSize:
					return new Size(m_sizeOriginal.Width, m_sizeOriginal.Height);
        ...
        private static DateTime m_ShowTime = DateTime.Now;
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (m.Msg == (WM_REFLECT + WM_COMMAND))
            {
                switch (HIWORD((int)m.WParam))
                {
                    ...
                    case CBN_CLOSEUP:
                        if ((DateTime.Now - m_ShowTime).Seconds > 1)
                            HideDropDown();
                        return;
                }
            }
...
//         }
		...
		{
			get { return m_popupCtrl == null ? false : m_popupCtrl.Cancelled; }
		}
