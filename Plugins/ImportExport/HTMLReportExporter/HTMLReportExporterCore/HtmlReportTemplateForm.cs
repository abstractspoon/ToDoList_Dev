...
namespace HTMLReportExporter
{
	...
	public partial class HtmlReportTemplateForm : Form
	{
		...
		protected override void OnLoad(EventArgs e)
		{
			...
			// IE version
			this.labelPreview.Text = String.Format("{0} (Internet Explorer {1})", m_Trans.Translate("Preview", Translator.CtrlType.Label), this.browserPreview.Version.Major);
		...
		private void OnOpenReportTemplate(object sender, EventArgs e)
		{
			var dlg = new OpenFileDialog
			{
				...
				Title = m_Trans.Translate("Open Report Template", Translator.CtrlType.Dialog),
		...
		{
			String title = m_Trans.Translate("Report Builder", Translator.CtrlType.Dialog), fileName = "";

			if (!String.IsNullOrEmpty(m_TemplateFilePath))
				fileName = Path.GetFileName(m_TemplateFilePath);
			else
				fileName = m_Trans.Translate("untitled", Translator.CtrlType.Text);
		...
		{
			get { return m_Trans.Translate("Report Templates (*.rbt)|*.rbt;", Translator.CtrlType.FileFilter); }
		}
		...
		private bool CheckSaveTemplate(bool mustHaveContents, bool newFileName)
		{
			...
			{
				//InitialDirectory = LastBrowsedFolder,
				Title = m_Trans.Translate("Save Report Template", Translator.CtrlType.Dialog),
		...
		private void OnInsertBackgroundImage(object sender, EventArgs e)
		{
			...
			dialog.BrowseTitle = m_Trans.Translate(dialog.BrowseTitle, Translator.CtrlType.Dialog);
			dialog.BrowseFilter = m_Trans.Translate(dialog.BrowseFilter, Translator.CtrlType.Dialog);
