...
namespace EisenhowerUIExtension
{
	...
	class TdlEisenhowerControl : EisenhowerControl_Org, IDragRenderer, ILabelTipHandler
	{
		...
		private Dictionary<UInt32, EisenhowerTaskItem> m_Items;
		...
		public TdlEisenhowerControl(Translator trans, UIExtension.TaskIcon icons)
		{
			m_Trans = trans;
			m_TaskIcons = icons;

			m_Items = new Dictionary<UInt32, EisenhowerTaskItem>();
		...
		{
//			Alignment = (EisenhowerControl.RootAlignment)prefs.GetProfileInt(key, "RootAlignment", (int)Alignment);
			Options = (EisenhowerOption)prefs.GetProfileInt(key, "Options", (int)Options);
		...
		private EisenhowerTaskItem TaskItem(TreeNode node)
		{
			if (node == null)
				return null;

			return (ItemData(node) as EisenhowerTaskItem);
		}

		private EisenhowerTaskItem RealTaskItem(TreeNode node)
		{
			return GetRealTaskItem(TaskItem(node));
		}

		private EisenhowerTaskItem TaskItem(Object itemData)
		{
			if (itemData == null)
				return null;

			return (itemData as EisenhowerTaskItem);
		}
			...
			if (newTask)
			{
				...
				if ((parentId == 0) && (parentNode == null))
				{
					...
					if (rootItem.IsTask)
					{
						...
						parentNode = AddRootNode(new EisenhowerTaskItem(m_Trans.Translate("Root", Translator.Type.Text)));
		...
		private void RebuildTreeView(TaskList tasks)
		{
			...
			if (taskIsRoot)
			{
                var taskItem = new EisenhowerTaskItem(task);
			...
			else
			{
				...
				var projName = GetProjectName(tasks);
				rootNode = AddRootNode(new EisenhowerTaskItem(projName));
...
// 				return !TaskItem(dropTargetItemData).IsLocked;
		...
		private EisenhowerTaskItem GetRealTaskItem(EisenhowerTaskItem taskItem)
		{
			if ((taskItem != null) && taskItem.IsReference)
			{
				EisenhowerTaskItem refItem;
		...
		protected void DrawItemLabel(Graphics graphics, EisenhowerTaskItem taskItem, Rectangle rect,
									  NodeDrawState nodeState, NodeDrawPos nodePos,
        ...
        CheckBoxState GetItemCheckboxState(EisenhowerTaskItem taskItem)
        {
		...
		private bool TaskHasIcon(EisenhowerTaskItem taskItem)
		{
		...
		protected override void DrawNodeConnection(Graphics graphics, Point ptFrom, Point ptTo)
		{
			...
			{
				if (m_Options.HasFlag(EisenhowerOption.StraightConnections))
				{
		...
		{
			if (m_Options.HasFlag(EisenhowerOption.ShowDependencies))
			{
		...
		protected void DrawTaskDependency(Graphics graphics, TreeNode nodeFrom, TreeNode nodeTo)
		{
			...
			EisenhowerItem itemFrom = Item(nodeFrom);
			EisenhowerItem itemTo = Item(nodeTo);
