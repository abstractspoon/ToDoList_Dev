...
namespace EisenhowerUIExtension
{
	...
	public delegate bool DragDropChangeEventHandler(object sender, EisenhowerDragEventArgs e);

	[System.ComponentModel.DesignerCategory("")]
	public partial class EisenhowerControl_Org : UserControl
    {
		// Constants ---------------------------------------------------------------------
		...
		private void DoDrop(TreeNode draggedNode, TreeNode dropTarget, DropPos dropPos, bool copy)
		{
            ...
            if (IsAcceptableDropTarget(draggedNode, dropTarget, dropPos, copy))
			{
				...
				var args = new EisenhowerDragEventArgs(draggedNode, parentNode, afterSiblingNode, copy);
		...
		virtual protected bool DoDrop(EisenhowerDragEventArgs e)
		{
            Debug.Assert(!ReadOnly);
		...
		protected void EnsureItemVisible(EisenhowerItem item)
        {
            if (item == null)
                return;
		...
		protected bool HandleCursorKey(Keys key)
		{
			...
			EisenhowerItem selItem = SelectedItem;
		...
		protected EisenhowerItem SelectedItem
		{
			get
			{
				if (SelectedNode == null)
		...
		protected EisenhowerItem Item(TreeNode node)
		{
			if (node == null)
				return null;

			return (node.Tag as EisenhowerItem);
		}
		...
		protected EisenhowerItem RootItem
		{
			get
			{
				if (IsEmpty())
        ...
        private TreeNode InsertNode(Object userData, TreeNodeCollection nodes, int pos, UInt32 uniqueID)
        {
            ...
            newNode.Tag = new EisenhowerItem(userData);
		...
		protected void RecalculatePositions()
		{
            ...
            EisenhowerItem rootItem = RootItem;
        ...
        private Rectangle CalculateChildBoundingBox(TreeNode node)
        {
            ...
            if (node.IsExpanded)
            {
                foreach (TreeNode child in node.Nodes)
                {
                    EisenhowerItem childItem = Item(child);
                    childBounds = EisenhowerItem.Union(childBounds, childItem.TotalBounds);
                }
            }
		...
		private Rectangle CalculateTotalBoundingBox(TreeNode fromNode, TreeNode toNode)
		{
			...
			{
				EisenhowerItem item = Item(node);
				totalBounds = EisenhowerItem.Union(totalBounds, item.TotalBounds);
			...
			case DropPos.Above:
				{
					...
					{
						EisenhowerItem prevItem = Item(node.PrevNode);
			...
			case DropPos.Below:
				{
					...
					{
						EisenhowerItem nextItem = Item(node.NextNode);
