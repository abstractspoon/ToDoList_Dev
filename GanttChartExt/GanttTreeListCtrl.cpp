...
BOOL CGanttTreeListCtrl::SaveToImage(CBitmap& bmImage)
{
	...
	CListCtrl* pList = (CListCtrl*)CWnd::FromHandle(m_hwndList);
	ASSERT(pList);

	if (!CCopyListCtrlContents(*pList).DoCopy(bmColumns))
		return FALSE;

	// Tasks header
	if (!CCopyHeaderCtrlContents(m_treeHeader).DoCopy(bmHeader))
		return FALSE;
