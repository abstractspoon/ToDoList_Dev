...
public: 
	...
	CString Convert(const CString& sHTML, const LOGFONT& lf, COLORREF clrText, COLORREF clrBack) 
	{ 
		// Create a new output buffer 
		// We assume that we can create a correctly sized output 
		// buffer before we convert HTML to RTF... 
		const int SIZE_FACTOR = 2; 
		unsigned int nTextSize = sHTML.GetLength(); 
		...
		m_clrBack = clrBack; 
		bool bRes = Parse(sHTML, pstrDest); 
		*pstrDest = _T('\0'); 
	...
	{ 
		...
		Context.sFaceName = m_aFonts[0].lfFaceName; 
         ...
         *pszAppendScope = _T('\0'); 
         ...
         if (bNewContext) { 
			 ...
			 if (_tcscmp(OldContext.sFaceName, Context.sFaceName)!=0) { 
				 AddOutputV(pstrDest, _T("\\f%d"), GetFontFace(Context.sFaceName)); 
				 bChanged = true; 
			 } 
