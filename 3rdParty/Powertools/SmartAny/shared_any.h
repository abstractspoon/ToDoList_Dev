...
// Get the declaration for IUnknown, but don't pull in all of windows.h
...
namespace detail
{
    ...
    struct nonintrusive
    {
        ...
        void swap( nonintrusive & that )
        {
            using std::swap;
            swap( m_t, that.m_t );
            swap( m_ref, that.m_ref );
        }
