...
CMap<HWND, HWND, CRichEditThemed*, CRichEditThemed*> CRichEditThemed::m_aInstances;
...
//	m_aInstances.erase(m_hRichEdit);
	m_aInstances.RemoveKey(m_hRichEdit);

//	if (m_aInstances.empty())
	if (m_aInstances.GetCount() == 0)
	{
		pOpenThemeData =  NULL;
...
//	if(itCurInstance != m_aInstances.end())
	...
	CRichEditThemed* pObj = NULL;

	if (CRichEditThemed::m_aInstances.Lookup(hwnd, pObj))
	{
