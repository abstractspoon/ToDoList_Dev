...
BEGIN_MESSAGE_MAP(CKanbanWnd, CDialog)
	...
	ON_UPDATE_COMMAND_UI_RANGE(1, 50, OnUpdateShortcuts)
	ON_WM_SHOWWINDOW()
...
void CKanbanWnd::OnUpdateShortcuts(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(TRUE);
	pCmdUI->SetCheck(pCmdUI->m_nID == (m_cbAttributes.GetCurSel() + 1));
}
...
BOOL CKanbanWnd::OnInitDialog() 
{
	...
	if (m_toolbar.CreateEx(this))
	{
		...
		int nAttrib = GetComboBoxItems(m_cbAttributes, aAttrib);

		int nBtn = m_toolbar.GetButtonCount();

		while (nBtn--)
			m_toolbar.SetButtonStyle(nBtn, m_toolbar.GetButtonStyle(nBtn) & ~0x0040);

		m_toolbar.ModifyStyle(0, TBSTYLE_LIST);
		m_toolbar.EnableMixedButtons();
		
		while (nAttrib--)
		{
			TBBUTTON tbb = { -1, nAttrib + 1, 0, TBSTYLE_BUTTON | TBBS_AUTOSIZE | 0x0040, 0, 0, (UINT)-1 };

			m_toolbar.GetToolBarCtrl().InsertButton(0, &tbb);	
			m_toolbar.SetButtonText(0, aAttrib[nAttrib]);
			m_toolbar.SetItemImage(0, -2);
		}
	}
