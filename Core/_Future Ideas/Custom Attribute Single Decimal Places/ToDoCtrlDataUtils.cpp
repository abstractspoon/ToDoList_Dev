									...
									const SEARCHPARAMS& query, SEARCHRESULT& result, BOOL bCheckDueToday) const
{
	...
	for (int nRule = 0; nRule < nNumRules && bMatches; nRule++)
	{
		...
		case TDCA_COMMENTSSIZE:
			bMatch = ValueMatches(pTDI->GetCommentsSizeInKB(), rule, resTask);
			break;
...
CString CTDCTaskFormatter::GetTimePeriod(double dTime, TDC_UNITS nUnits, BOOL bAllowNegative, int nDecPlaces) const
{
	...
	if (nDecPlaces == -1)
		nDecPlaces = (m_data.HasStyle(TDCS_ROUNDTIMEFRACTIONS) ? 0 : 2);

	if (m_data.HasStyle(TDCS_DISPLAYHMSTIMEFORMAT))
	{
		DWORD dwFlags = (nDecPlaces ? HMS_DECIMALPLACES : 0);
	...
	case TDCCA_TIMEPERIOD:
		{
			...
			int nDecPlaces = (attribDef.HasFeature(TDCCAF_SINGLEDECIMAL) ? 1 : 2);

			return GetTimePeriod(dValue, nUnits, TRUE, nDecPlaces);
		}
		break;
