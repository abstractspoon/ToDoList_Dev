...
BOOL CToDoCtrlData::TaskHasStartDueDates(DWORD dwTaskID) const
{
	return GetTaskStartDueDates(dwTaskID, COleDateTimeRange());
}
...
int CToDoCtrlData::GetTaskSubtaskIDs(DWORD dwTaskID, CDWordArray& aSubtaskIDs, BOOL bAppend) const
{
	const TODOSTRUCTURE* pTDS = NULL;
	GET_TDS(dwTaskID, pTDS, 0);
	...
	if (pTDS->HasSubTasks())
	{
		int nNumSubtasks = pTDS->GetSubTaskCount();
		aSubtaskIDs.SetSize(nNumSubtasks);

		for (int nSubtask = 0; nSubtask < nNumSubtasks; nSubtask++)
			aSubtaskIDs[nSubtask] = pTDS->GetSubTaskID(nSubtask);
	}
	else if (!bAppend)
	{
