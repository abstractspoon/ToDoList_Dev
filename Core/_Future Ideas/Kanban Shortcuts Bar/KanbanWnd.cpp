...
void CKanbanWnd::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_TAB1, m_tcShortcutBar);
...
BOOL CKanbanWnd::OnInitDialog() 
{
	...
	while (nAttrib--)
		m_tcShortcutBar.InsertItem(0, aAttrib[nAttrib], -1);
...
void CKanbanWnd::Resize(int cx, int cy)
{
	if (m_ctrlKanban.GetSafeHwnd())
	{
		...
		ResizeChild(&m_tcShortcutBar, cx, 0);
		...
		rMain.top = CDlgUnits(this).ToPixelsY(43);
