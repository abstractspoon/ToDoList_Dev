										  ...
										  COLORREF crText, BOOL bSelected)
{
	...
	for (int nCol = 1; nCol < nNumCol; nCol++)
	{
		...
		DrawColumn(pDC, dwTaskID, pTDI, pTDS, *pCol, rSubItem, crText);
	}
}

void CTDLTaskCtrlBase::DrawColumn(CDC* pDC, DWORD dwTaskID, const TODOITEM* pTDI, const TODOSTRUCTURE* pTDS, 
	...
	case TDCC_PRIORITY:
		...
		if (!HasStyle(TDCS_DONEHAVELOWESTPRIORITY) || !m_calculator.IsTaskDone(pTDI, pTDS))
		{
			...
			if (bHasPriority && !HasStyle(TDCS_HIDEPRIORITYNUMBER))
			{
				...
				DrawColumnText(pDC, sTaskColText, rPriority, col.nTextAlignment, crTemp);
			}
		}
		break;
			...
			if (HasStyle(TDCS_SHOWPERCENTASPROGRESSBAR))
			{
				...
				if (HasColor(crBar))
				{
					...
					if (rProgress.Width() > 0)
					{
						...
						DrawColumnText(pDC, sTaskColText, rPercent, col.nTextAlignment, crTemp);
					}
				}
			}
