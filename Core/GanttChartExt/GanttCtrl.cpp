...
BOOL CGanttCtrl::GetListColumnDate(int nCol, COleDateTime& date, BOOL bEndOfMonth) const
{
	...
	date = CDateHelper::GetDateFromMonths(CDateHelper::GetDateInMonths(nMonth, nYear));
...
int CGanttCtrl::GetDependencyListItem(DWORD dwTaskID) const
{
	HTREEITEM hti = GetTreeItem(dwTaskID);

	// Use the parent as a proxy if collapsed
...
void CGanttCtrl::DrawGanttDone(CDC* pDC, const CRect& rMonth, int nMonth, int nYear, const GANTTITEM& gi, BOOL bSelected)
{
	...
	if (!CalcDateRect(rMonth, dtMonthStart, dtMonthEnd, gi.dtDone, gi.dtDone, rDone))
		return;
...
BOOL CGanttCtrl::DrawToday(CDC* pDC, const CRect& rMonth, int nMonth, int nYear, BOOL bSelected)
{
	...
	if (!CalcDateRect(rMonth, dtMonthStart, dtMonthEnd, dtToday, dtToday.m_dt + 1.0, rToday))
		return FALSE;
...
void CGanttCtrl::UpdateListColumnsWidthAndText(int nWidth)
{
	...
	do
	{
		...
		if (nMonth && nYear)
		{
			...
			m_listHeader.SetItem(nCol, nColWidth, sTitle, nNumMonths);
