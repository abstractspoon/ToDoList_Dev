...
void CXPTabCtrl::InitImageList(UINT nBitmapID)
{
	...
	if (!::IsWindow(GetSafeHwnd()) || m_ilTabs.GetSafeHandle())
	{
		ASSERT(FALSE); 
		return;
	}				
	
	// tab control has to be created first and image list can be created only once
...
void CXPTabCtrl::DoPaint(CDC* pDC)
{
	...
	case e_tabBottom: 
		rcClient.bottom = rcPage.bottom + 3;
		break;

	case e_tabLeft:	  
		rcClient.left = rcPage.left - 1;
		break;

	case e_tabRight:  
		rcClient.right = rcPage.right + 3;
		break;
	...
	for (int ixTab = 0; ixTab < nTab; ixTab++)
	{
		...
		if (m_eTabOrientation == e_tabLeft) 
			rcItem.right++;
...
{			
	...
	if (bBody)
		...
		DrawThemesPart(dcMem.GetSafeHdc(), 1, bSel ? 3 : (bHot ? 2 : 1), (LPCTSTR)IDS_UTIL_TAB, &rcMem);
	...
	if (bVertic) // force rotating bitmap as RGB -> good for any resolution
	{
		...
		LPBYTE pcImgRotate = new BYTE[nSzBuffPD]; 
		ASSERT(pcImgRotate);
...
#define __USE_MASM__
		...
		SetDIBits(*pDC, bmpMem, 0, szBmp.cy, pcImgRotate, &biOut, DIB_RGB_COLORS); // set rotated bitmap bits
		delete pcImgRotate;
	}
...
BOOL IsWinXP() 
{ 
	return GetWinVersion() >= PACKVERSION(5, 1); 
}

BOOL CXPTabCtrl::IsThemeActiveXP() 
{ 
	return (IsWinXP() && IsThemeActiveEx()); 
}
