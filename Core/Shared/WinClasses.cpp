...
BOOL CWinClasses::IsMFCPropertyPage(HWND hWnd)
{
...
BOOL CWinClasses::IsMFCCommonDialog(HWND hWnd, WCLS_COMMONDIALOG nType)
{
	// special case
	if (nType == WCD_ANY)
	{
		return IsMFCCommonDialog(hWnd, WCD_PRINT) ||
				IsMFCCommonDialog(hWnd, WCD_PAGESETUP) ||
				IsMFCCommonDialog(hWnd, WCD_FONT) ||
...
BOOL CWinClasses::IsWinFormsControl(LPCTSTR szClass, LPCTSTR szWndClass)
{
	...
	// else
	return (Misc::Find(szWndClass, szClass, FALSE, TRUE) != -1);
}

BOOL CWinClasses::IsWinFormsControl(HWND hWnd, LPCTSTR szWndClass)
{
	return IsWinFormsControl(GetClass(hWnd), szWndClass);
}
