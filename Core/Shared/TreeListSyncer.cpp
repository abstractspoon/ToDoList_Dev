...
BOOL CTreeListSyncer::HandleMouseWheel(HWND hWnd, WPARAM wp, LPARAM lp)
{
	...
	if (hwndTree)
	{
		// There's a very strange bug (probably caused somehow 
		// by the following method) where during a long, smooth
		// scroll, the scroll direction suddenly reverses.
		...
		const int MAX_ROWS_PER_CLICK = 8;

		int nRowsPerClick = 3, nTotalRows = (nNumClicks * nRowsPerClick);

		while ((nNumClicks > MAX_CLICKS) && (nRowsPerClick < MAX_ROWS_PER_CLICK))
		{
			nRowsPerClick++;
			nNumClicks = ((nTotalRows / nRowsPerClick) + 1);
		}
		...
		while (nNumClicks--)
		{
			...
			for (int nRow = 0; nRow < nRowsPerClick; nRow++)
			{
