	...
	: CRuntimeDlg(), m_bConfirm(bConfirm)
{
    ...
    AddRCControl(_T("EDITTEXT"), _T(""), _T(""), WS_TABSTOP | ES_PASSWORD | ES_AUTOHSCROLL, 0, EDITSTART,7 + YEXPOFFSET,110,13, IDC_PD_PASSWORD);

	if (m_bConfirm)
	{
		AddRCControl(_T("LTEXT"), _T(""), sConfirm, 0, 0,7, 28 + YEXPOFFSET,PROMPTLEN,8, IDC_PD_CONFIRMLABEL);
		AddRCControl(_T("EDITTEXT"), _T(""), _T(""), WS_TABSTOP | ES_PASSWORD | ES_AUTOHSCROLL, 0, EDITSTART,25 + YEXPOFFSET,110,13, IDC_PD_CONFIRM);
	}
